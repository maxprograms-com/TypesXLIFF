...
export class XliffTarget implements XliffElement {

    readonly elementName: string = "target";
    xmlLang?: string;
    ...
    getOrder(): number | string | undefined {
        return this.order;
    }

    setOrder(order: number | string | undefined): void {
        this.order = order;
    }
    ...
    toElement(): XMLElement {
        ...
        if (this.order !== undefined) {
            element.setAttribute(new XMLAttribute('order', String(this.order)));
        }
